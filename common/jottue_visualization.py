...
def render_animation(keypoints, keypoints_metadata, poses, skeleton, fps, bitrate, azim, output, viewport,
                     limit=-1, downsample=1, size=6, input_video_frames=None, input_video_skip=0):
    """
    TODO
    ...
    ##################### make video frame list ######################
    # Decode video
    if input_video_frames is None:
        # Black background
        all_frames = np.zeros((keypoints.shape[0], viewport[1], viewport[0]), dtype='uint8')
    else:
        all_frames = input_video_frames.astype(np.uint8)
